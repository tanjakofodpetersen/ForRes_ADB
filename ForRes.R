...
ggplot(data = ferdig %>% filter(!Kategori2023 == "NR")  # Tag bort arter som per definisjon ikke har en etableringsklasse
...
ggplot(data = ferdig %>%
         filter(!Kategori2023 == "NR") %>% # Tag bort arter som per definisjon ikke har utslagsgivende kriterier
...
ferdig %>%
  filter(#Vurderingsomraade == "N",
         #Ekspertkomite == "Karplanter",
         Fremmedartsstatus == "Selvstendig reproduserende" | Fremmedartsstatus == "Doerstokkart",
         !is.na(effektKrit),
...
ferdig %>% filter(Etableringsklasse == "") %>% View()
